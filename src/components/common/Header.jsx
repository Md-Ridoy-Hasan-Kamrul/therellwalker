...
export const Header = ({ title, onMenuClick }) => {
  ...
  // Priority: localStorage userName > user.username > user.fname > user.name
  const localUserName = localStorage.getItem('userName');
  const userName =
    localUserName ||
    user?.username ||
    (user?.fname && user?.fname !== 'null' ? user?.fname : null) ||
    ...
    <header className='w-full px-4 sm:px-6 md:px-8 lg:px-11 py-4 sm:py-5 bg-gradient-to-t from-[#2C223B] to-[#892E6D] border-b border-slate-100/25 flex justify-between items-center gap-4'>
      ...
      >
        ...
        <span
          className={`${
            isDropdownOpen ? 'rotate-180 ' : ''
          } -ml-2 sm:hidden transition-all duration-300`}
        >
          {' '}
          <IoCaretDownOutline />
        </span>
        ...
        {/* Dropdown Menu */}
        <div
          className={`absolute top-full right-0 mt-2 w-56 bg-neutral-800 rounded-lg shadow-lg border border-white/10 overflow-hidden z-50 transform transition-all duration-300 origin-top
    ${
      isDropdownOpen
